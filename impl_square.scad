 ...
 * Connector Negative
 */
module connector_negative_square(x,y,square_basis,edge_width,north=true,south=true,east=true,west=true) {
    for ( i = [0 : y-1] ) {
        if(west) {
            translate([0,square_basis*(i+1)-square_basis/2,0]) center_connector_negative(y, PRIORITY, LOCK, MAGNETS, MAGNET_HOLE);
        }
        if(east) {
            translate([square_basis*x,square_basis*(i+1)-square_basis/2,0]) rotate([0,0,180]) center_connector_negative(y, PRIORITY, LOCK, MAGNETS, MAGNET_HOLE);
        }
    }
    if (y > 1) {
        for ( i = [1 : y-1] ) {
            ...
            if(east) {
                translate([square_basis*x,square_basis*i,0]) rotate([0,0,180]) joint_connector_negative(y);
            }
        }
    }
    for ( i = [0 : x-1] ) {
        if(south) {
            translate([square_basis*(i+1)-square_basis/2,0,0]) rotate([0,0,90]) center_connector_negative(x, PRIORITY, LOCK, MAGNETS, MAGNET_HOLE);
        }
        if(north) {
            translate([square_basis*(i+1)-square_basis/2,square_basis*y,0]) rotate([0,0,-90]) center_connector_negative(x, PRIORITY, LOCK, MAGNETS, MAGNET_HOLE);
        }
    }
    if (x > 1) {
        for ( i = [1 : x-1] ) {
            if(south) {
                translate([square_basis*i,0,0]) rotate([0,0,90]) joint_connector_negative(y);
            }
            if(north) {
                translate([square_basis*i,square_basis*y,0]) rotate([0,0,-90]) joint_connector_negative(y);
            }
        }
    }
}
...
module plain_square_negative(x,y,square_basis,edge_width) {
    difference() {
        ...
        if (CENTER == "true") {
            for ( i = [2 : 2 : y-1] ) {
                translate([0,(i-.125)*square_basis,-2]) cube([square_basis*x,square_basis/4,10]);
            }
            for ( i = [2 : 2 : y-1] ) {
                translate([(i-.125)*square_basis,0,-2]) cube([square_basis/4,square_basis*y,10]);
            }
        }
    }
}
